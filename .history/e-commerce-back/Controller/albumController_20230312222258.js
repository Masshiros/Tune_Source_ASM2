...
exports.getAlbumByID = async (req, res, next, id) => {
  try {
    const album = await Album.findById(id).populate("genreId");
    if (!album) {
      return res.status(400).json({
...
exports.listSearch = async (req, res) => {
  ...
  if (req.query.search) {
    ...
    // assign genreId value to query.genreId
    if (req.query.genreId && req.query.genreId != "All") {
      query.genreId = req.query.genreId;
    }
  }
  console.log(query);
  // find the album based on query object
  try {
    const albums = await Album.find(query).select("-photo");
    res.json(albums);
  } catch (err) {
    if (err) {
      console.log(err);
